...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Trigger role fetching in context
      await loginSuccess();
      setLoginTriggered(true); // Mark that login attempt happened
    } catch (err) {
      setError(
  ...
  // Watch for role change and navigate
  useEffect(() => {
    if (!loginTriggered) return; // only navigate after login attempt
    if (!role) return; // still loading role

    if (role === "admin") navigate("/admin");
    else if (role === "storekeeper") navigate("/store");
    else setError("Unauthorized role");
  }, [role, loginTriggered, navigate]);
...
//       // Trigger fetchUserRole in context
//       await loginSuccess();
...
//       else if (userRole === "storekeeper") navigate("/store");
//       else setError("Unauthorized role");
//     } catch (err) {
//       setError(
//         err.response?.data?.message || "Login failed. Please check credentials."
